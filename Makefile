...
flake: ## check style with flake8
	flake8 src/frkl/project_meta tests

mypy: ## run mypy
	mypy src/frkl/project_meta

check: black flake mypy test ## run dev-related checks

project-info:
	mkdir -p .frkl
	frkl-project metadata frkl.project_meta > .frkl/project.json
	frkl-project pyinstaller-args frkl.project_meta > .frkl/pyinstaller-args.json
...
attribution: ## create a json file with attribution information
	scripts/attribution/create_attribution_data.sh . ./src/frkl/project_meta/resources/attribution.json
